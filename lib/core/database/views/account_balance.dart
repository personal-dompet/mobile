...
CREATE VIEW $accountBalanceView AS
  SELECT 
    $accountTable.*,
    CASE
      WHEN $accountTable.${AccountKey.normalBalance} = '${BalanceType.debit.value}' THEN
        SUM(CASE WHEN $journalEntryTable.${JournalEntryKey.status} = '${JournalStatus.posted.name}' THEN $journalLineTable.${JournalLineKey.debitAmount} ELSE 0 END) - 
        SUM(CASE WHEN $journalEntryTable.${JournalEntryKey.status} = '${JournalStatus.posted.name}' THEN $journalLineTable.${JournalLineKey.creditAmount} ELSE 0 END)
      ELSE
        SUM(CASE WHEN $journalEntryTable.${JournalEntryKey.status} = '${JournalStatus.posted.name}' THEN $journalLineTable.${JournalLineKey.creditAmount} ELSE 0 END) - 
        SUM(CASE WHEN $journalEntryTable.${JournalEntryKey.status} = '${JournalStatus.posted.name}' THEN $journalLineTable.${JournalLineKey.debitAmount} ELSE 0 END)
